...
namespace AutomacaoPromobTeste{
    internal class Program{
        ...
        static void Main(string[] args){
            ...
            if (arquivos.Count == 0){
                Log("[ERRO] Nenhum arquivo .promob encontrado na pasta.", LogLevel.Error);
                Console.ReadKey();
                return;
            }

            Log($"[INFO] {arquivos.Count} arquivo(s) encontrado(s).\n");
            ...
            foreach (var arquivo in arquivos){
                ...
                Console.WriteLine("══════════════════════════════════════════");
                Console.WriteLine($"[{processados + erros + 1}/{arquivos.Count}] Processando: {nome}");
                Console.WriteLine("══════════════════════════════════════════");
            ...
            Console.WriteLine("╠══════════════════════════════════════════╣");
            Console.WriteLine($"║  Total:       {arquivos.Count,-27}║");
            Console.WriteLine($"║  Processados: {processados,-27}║");
            Console.WriteLine($"║  Erros:       {erros,-27}║");
            Console.WriteLine("╚══════════════════════════════════════════╝");
        ...
        static void ClicarBotaoImportar(Window janelaPromob){
            if (ElementoValido(_cachedBotaoImportar)){
                try{
                    Log("  [OK] Botão 'Importar' encontrado no cache.");
            ...
            Log($"  [INFO] Procurando botão pelo AutomationId '{AutomationIdImportar}'...");

            var btnFound = buscaEm.FindFirstDescendant(cf => cf.ByAutomationId(AutomationIdImportar));

            if (btnFound == null){
                Log("  [AVISO] Busca direta falhou. Tentando fallback...", LogLevel.Warn);
            ...
            if (btnFound == null)
                throw new Exception($"Botão '{AutomationIdImportar}' não encontrado no layout.");

            Log($"  [OK] Botão encontrado! Nome: '{btnFound.Name}', Id: '{btnFound.AutomationId}'");
        ...
        static void PreencherDialogoNativo(UIA3Automation automation, string caminhoCompleto, Window dialogo){
            ...
            Log($"  [INFO] Copiando nome do arquivo: {nomeArquivo}");

            // copia o nome para o clipboard
            CopiarParaClipboardNativo(nomeArquivo);

            // pequeno respiro para garantir que o clipboard foi atualizado
        ...
        static void AbrirProjetoSelecionado(Window janelaPromob, string nomeProjeto){
            ...
            // Loop de espera dinâmico solicitado: ciclos de 60s procurando o botão
            while (true){
                Log("  [INFO] Aguardando o carregamento do projeto (ciclo de 60s)...");

                bool carregou = EsperarAte(() =>{
                    var host = janelaPromob.FindFirstDescendant(AutomationIdHost);
                    var aba = janelaPromob.FindFirstDescendant(cf => 
                        cf.ByAutomationId("ToolsTab").Or(cf.ByName("Ferramentas")));
                    
                    var pronto = host != null || aba != null;
                    
                    if (!pronto){
                        // Check for blocking popups during load
                        var popup = EncontrarPopupAtencao(janelaPromob.Automation.GetDesktop());
                        if (popup != null) TratarPopupGenerico(popup);
                    }
                    else if (aba != null){
                        Log("  [OK] Aba 'Ferramentas' detectada durante a espera. Selecionando...");
                        SelecionarOuClicar(aba);
                    }

                    return pronto;
                }, timeoutMs: 60000, intervaloMs: 2000);

                if (carregou){
                    Log("  [OK] Projeto carregado e aba selecionada.");
                    EsperarUiRespirar(1000);
                    break; 
                }

                Log("  [AVISO] Timeout de 60s atingido. Realizando varredura final...");
                var abaFinal = janelaPromob.FindFirstDescendant(cf => 
                    cf.ByAutomationId("ToolsTab").Or(cf.ByName("Ferramentas")));

                if (abaFinal != null){
                    Log("  [OK] Aba encontrada na varredura final!");
                    SelecionarOuClicar(abaFinal);
                    break;
                }

                Log("  [AVISO] Projeto ainda não carregado. Iniciando novo ciclo de 60s...", LogLevel.Warn);
            }
        }
        ...
        static void ClicarComFallback(AutomationElement el){
            ...
            // Tenta garantir que a janela pai esteja ativa antes de clicar
            try { 
                var win = el.AsWindow();
                if (win != null) AtivarJanela(win);
            } catch { }
        ...
        static bool TentarDefinirValor(AutomationElement el, string valor){
            // Tenta garantir foco antes de definir valor
            try { 
                var win = el.AsWindow();
                if (win != null) AtivarJanela(win);
                Focar(el);
